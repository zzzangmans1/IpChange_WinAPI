...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN:
        ...
        if (GetKeyState(VK_CONTROL) & 0x8000) {
            MessageBox(hWnd, L"Ctrl + A 눌림", L"알림", MB_OK);
        }
        break;
    case WM_COMMAND:
        {
            ...
            if (HIWORD(wParam) == EN_SETFOCUS) {
                // EDIT 컨트롤에 포커스가 올 때 처리할 내용 (필요하면 추가)
                hFocus = GetFocus(); // 현재 포커스를 가진 윈도우 핸들
                //wchar_t buffer[100];

                // 핸들을 문자열로 변환
                //swprintf(buffer, 100, L"현재 포커스 핸들: 0x%p", hFocus);
                TCHAR className[256];
                if (hFocus && GetClassName(hFocus, className, sizeof(className) / sizeof(TCHAR))) {
                    
                    //SendMessage(hFocus, EM_SETSEL, 0, -1); // 모든 텍스트 선택
                    if (lstrcmpi(className, L"Edit") > 0 ) {
                        MessageBox(NULL, className, L"현재 포커스된 컨트롤의 클래스가 EDIT이 아닙니다.", MB_OK);
                    }
                    else if (lstrcmpi(className, L"Edit") == 0) {
                        //MessageBox(NULL, className, L"현재 포커스된 컨트롤의 클래스가 EDIT입니다.", MB_OK);
                        SendMessage(hFocus, EN_SETFOCUS, 0, -1);
                    }
                }
                else {
                    MessageBox(NULL, L"포커스된 컨트롤 없음 또는 오류 발생", L"오류", MB_OK);
                }
                // MessageBox로 출력
                //MessageBox(hWnd, buffer, L"알림", MB_OK);
            }
            break;
            // 메뉴 선택을 구문 분석합니다:
            switch (wmId)
